...
RimeSessionHolder::RimeSessionHolder(RimeSessionPool* pool, const std::string &program) : pool_(pool) {
...
void RimeSessionPool::setPropertyPropagatePolicy(PropertyPropagatePolicy policy) {
    if (policy_ == policy) {
        return;
    }
...
std::shared_ptr<RimeSessionHolder> RimeSessionPool::requestSession(InputContext *ic) {
    if (!engine_->api()) {
        return nullptr;
    }
    std::string key;
    ...
    try {
        auto newSession = std::make_shared<RimeSessionHolder>(this, ic->program());
...
void RimeSessionPool::registerSession(const std::string &key, std::shared_ptr<RimeSessionHolder> session) {
    assert(!key.empty());
    session->key_ = key;
